...
const CarouselVideoCard = ({ card, isCenter, index }: CarouselVideoCardProps) => {
  ...
  useEffect(() => {
    ...
    if (isCenter) {
      ...
      try {
        ...
        // ignore errors if video is not fully ready
      }
    }
  }, [isCenter]);

  const isVideo = card.linkUrl && card.linkUrl.endsWith(".mp4");
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  // 1. Auto-Play Effect (2 seconds, paused on hover or overlay active)
  useEffect(() => {
    if (!needsPagination || isHovered || activeVideoUrl !== null) return;
    const interval = setInterval(() => {
      cycle("right");
    }, 2000);
    return () => clearInterval(interval);
  }, [cycle, needsPagination, isHovered, centerIndex, activeVideoUrl]);

  // 2. Drag & Touch Swipe Event Listener Effect with Click Prevention
  useEffect(() => {
    ...
    const handleDragMove = (x: number) => {
      ...
      if (Math.abs(diff) > 55) {
        ...
        isDragging = false; // Trigger once per drag gesture
      }
    };
  ...
  return (
    <section className="fan-section flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="fan-container flex items-center justify-center w-full max-w-[90rem]">
        <div
          ref={containerRef}
        ...
        >
          {cards.map((card, index) => {
            ...
            const image = (
              <CarouselVideoCard card={card} isCenter={isCenter} index={index} />
            );

            const isVideo = card.linkUrl && card.linkUrl.endsWith(".mp4");
            return card.linkUrl && !isVideo ? (
              <a
                key={index}
                href={card.linkUrl}
                ...
                onClick={(e) => {
                  if (isAnimating.current || hasDragged.current) {
                    e.preventDefault();
                  }
                }}
              >
              ...
              <div
                ...
                className="fan-card block cursor-pointer"
                onClick={() => {
                  if (!isAnimating.current && !hasDragged.current && isVideo) {
                    setActiveVideoUrl(card.linkUrl || null);
                  }
                }}
              >
                {image}
      ...
      {needsPagination && (
        <div className="fan-pagination-container">
          <button className={ARROW_CLASSES} onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="fan-dots-container">
            {cards.map((_, i) => (
      ...
      {activeVideoUrl && (
        <div className="fan-lightbox-overlay" onClick={() => setActiveVideoUrl(null)}>
          <div className="fan-lightbox-content" onClick={(e) => e.stopPropagation()}>
            <button className="fan-lightbox-close" onClick={() => setActiveVideoUrl(null)} aria-label="Close Preview">
              &times;
            </button>
            <video
              src={activeVideoUrl}
